...
fun Module.containerRepository() {
    ...
    single<SqlClient> {
        ...
        val jdbcConnectOptions = JDBCConnectOptions() // <--- Usar JDBCConnectOptions
            .setJdbcUrl(config.getString("db.container.url"))
            .setUser(config.getString("db.container.user"))
            .setPassword(config.getString("db.container.password"))
        val poolOptions = PoolOptions().setMaxSize(config.getInt("db.container.pool_max_size"))

        val client = JDBCPool.pool(vertx, jdbcConnectOptions, poolOptions)
        client.query(loadSqlScript("db/migrations/V1__container_initial_schema.sql")).execute()
            .onComplete { ar ->
                if (ar.succeeded()) {
                    log.info("ContainerService DB schema created successfully.")
                } else {
                    log.error("Failed to create ContainerService DB schema: ${ar.cause().message}", ar.cause())
                }
            }
        client
    }
    single<ContainerRepository> {
        val sqlClient = get<SqlClient>()
        ContainerRepositoryImpl(sqlClient)
    }
}
